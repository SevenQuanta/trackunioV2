...
ax25_send_header(const struct s_address *addresses, int num_addresses)
{
  ...
  Serial3.print("] ");
  Serial3.print(addresses[1].callsign);
  if (addresses[1].ssid) {
    Serial3.print('-');
    Serial3.print((unsigned int)addresses[1].ssid);
  }
  Serial3.print('>');
  // Destination callsign
  Serial3.print(addresses[0].callsign);
  if (addresses[0].ssid) {
    Serial3.print('-');
    Serial3.print((unsigned int)addresses[0].ssid);
  }
  for (i = 2; i < num_addresses; i++) {
    Serial3.print(',');
    Serial3.print(addresses[i].callsign);
    if (addresses[i].ssid) {
      Serial3.print('-');
      Serial3.print((unsigned int)addresses[i].ssid);
    }
  }
  Serial3.print(':');
#endif
